...
class ServiceConnection(private val context: Context, callback: Callback, private val register: Boolean = true) : ServiceConnection {
    ...
    override fun onBindingDied(name: ComponentName?) {
        // Do not auto-reconnect when VPN was revoked by another app or service stopped cleanly.
        if (status == Status.Stopped || !Settings.startedByUser) {
            Log.d(TAG, "service dead — not reconnecting (stopped or not user-started)")
            return
        }
        reconnect()
        Log.d(TAG, "service dead — reconnecting")
    }
