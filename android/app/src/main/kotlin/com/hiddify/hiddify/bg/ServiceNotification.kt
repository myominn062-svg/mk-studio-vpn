...
import com.hiddify.hiddify.constant.MkStudioIdentifiers
...
class ServiceNotification(private val status: MutableLiveData<Status>, private val service: Service) : BroadcastReceiver(){
    companion object {
        private const val notificationId = MkStudioIdentifiers.NOTIFICATION_ID_SERVICE
        private const val notificationChannel = MkStudioIdentifiers.NOTIFICATION_CHANNEL_SERVICE
    ...
    fun show(profileName: String, @StringRes contentTextId: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            Application.notification.createNotificationChannel(
                NotificationChannel(
                    notificationChannel, "MK Studio VPN service", NotificationManager.IMPORTANCE_LOW
                )
            )
        }
        service.startForeground(
            notificationId, notificationBuilder
                .setContentTitle(profileName.takeIf { it.isNotBlank() } ?: "MK Studio VPN")
