...
[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\MK Studio\MK Studio VPN"
Type: filesandordirs; Name: "{userappdata}\MK Studio VPN"
...
var
  ResultCode: Integer;
begin
  { Stop running MK Studio / legacy Hiddify processes before overwrite }
  Exec('taskkill', '/F /IM MKStudioVPN.exe', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)
  Exec('taskkill', '/F /IM hiddify.exe', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)
  Exec('net', 'stop "MKStudioVPNTunnelService"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)
  Exec('sc.exe', 'delete "MKStudioVPNTunnelService"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode)
  { Prebuilt cores may still register the upstream service name }
