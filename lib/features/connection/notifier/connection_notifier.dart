...
class ConnectionNotifier extends _$ConnectionNotifier with AppLogger {
  ...
  void _startConnectTimeout() {
    ...
    _connectTimeoutTimer = Timer(const Duration(seconds: 45), () {
      loggy.warning("connect timeout — aborting");
      _disconnect(force: true);
    });
  }
  ...
  final _operationLock = ConnectionOperationLock();

  Future<void> _connect() async {
    await _operationLock.runConnect(() => _connectThrottled());
  }
  ...
  Future<void> _disconnect({bool force = false}) async {
    _cancelConnectTimeout();
    await _operationLock.runDisconnect(
      () async {
...
class ConnectionOperationLock {
  bool _running = false;
  bool _disconnectRequested = false;

  Future<void> runConnect(Future<void> Function() task) async {
    if (_running) return;
    _running = true;
  ...
  Future<void> runDisconnect(Future<void> Function() task, {bool force = false}) async {
    if (_running && !force) {
