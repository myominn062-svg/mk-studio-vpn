...
class HiddifyCoreService with InfraLogger {
      ...
      for (var attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          if (res.messageType != MessageType.ALREADY_STARTED && res.messageType != MessageType.EMPTY) {
            ...
            final alert = msg.contains("denied") || msg.contains("vpn permission")
          ...
          loggy.error("failed to start bg core (attempt ${attempt + 1}): $e");
          if (attempt == 0 && e.code == StatusCode.unavailable) {
            loggy.info("retrying background core setup after gRPC unavailable");
            await Future.delayed(const Duration(milliseconds: 900));
            final retryBackground = await core.setupBackground(path, name);
            if (retryBackground != const CoreStatus.started()) {
              statusController.add(currentState = const CoreStatus.stopped());
              return left(
                retryBackground.getCoreAlert() ??
                    const ConnectionFailure.backgroundCoreNotAvailable(
                      "VPN background service did not start. Please try again.",
                    ),
              );
            }
            continue;
          ...
          ref.read(coreRestartSignalProvider.notifier).restart();
          statusController.add(currentState = const CoreStatus.stopped());
          return left(_mapBgStartGrpcError(e));
        }
      }
  ...
  Future<void> closeFront() async {
    ...
    if (!core.isSingleChannel()) {
      // Keep bg listeners alive — VPN foreground service keeps running.
  ...
  /// Re-sync UI state after returning from background without tearing down VPN.
  ConnectionFailure _mapBgStartGrpcError(GrpcError e) {
    final msg = (e.message ?? '').toLowerCase();
    if (msg.contains('vpn permission') || msg.contains('missing vpn') || msg.contains('denied')) {
      return ConnectionFailure.missingVpnPermission(
        e.message ?? 'Allow VPN permission when Android asks, then tap Connect again.',
      );
    }
    if (e.code == StatusCode.unavailable) {
      return const ConnectionFailure.backgroundCoreNotAvailable(
        'VPN background service is still starting. Wait a few seconds and try again.',
      );
    }
    return ConnectionFailure.unexpected("failed to start background core: ${e.message ?? e.codeName}");
  }

  Future<void> restoreAfterForeground() async {
    if (!core.isInitialized()) {
      await init();
      return;
    }

    final bgActive = await core.isActiveBg();
    if (bgActive) {
    ...
    try {
      ...
      loggy.debug("foreground restore setup skipped: $e");
    }
  }
}
