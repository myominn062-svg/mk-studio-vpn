...
windows-zip-release:
	...
	mkdir -p MKStudioVPN; \
	unzip -q "$$ZIP_FILE" -d MKStudioVPN/; \
	rm "$$ZIP_FILE"; \
	tar -a -cf "$$FILE_NAME.zip" MKStudioVPN; \
	rm -rf MKStudioVPN; \
...
linux-appimage-release:
	...
	$(BLUE)Adding StartupWMClass to mkstudiovpn.desktop$(DONE); \
	sed -i '/^\[Desktop Entry\]/a StartupWMClass=com.mkstudio.vpn' "squashfs-root/mkstudiovpn.desktop"; \
	...
	ARCH=x86_64 appimagetool --no-appstream squashfs-root MK-Studio-VPN.AppImage > /dev/null; \
	$(BLUE)Cleaning up squashfs$(DONE); \
	rm -rf squashfs-root; \
	$(YELLOW)Creating Portable Package$(DONE); \
	PKG_DIR_NAME="mk-studio-vpn-linux-appimage"; \
	$(BLUE)Creating dir: $$PKG_DIR_NAME$(DONE); \
	mkdir -p "$$PKG_DIR_NAME"; \
	$(BLUE)Moving MK-Studio-VPN.AppImage$(DONE); \
	cp -p "MK-Studio-VPN.AppImage" "$$PKG_DIR_NAME/MK-Studio-VPN.AppImage"; \
	$(BLUE)Creating Portable Home directory$(DONE); \
	mkdir -p "$$PKG_DIR_NAME/MK-Studio-VPN.AppImage.home"; \
